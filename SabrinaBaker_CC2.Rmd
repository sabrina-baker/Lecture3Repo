...
author: "Group 6"
date: "`r format(Sys.Date(), '24 September 2026')`"
output: 
  html_document:
    toc: true
...
1. **Sabrina Baker** — 20449887
 + [BIOL343_CC](https://github.com/sabrina-baker/BIOL343_CC02) 
2. **Victoria Gutierrez** — 20517120
 + [BIOL343_Victoria-GA_CC2](https://github.com/Victoria-GA/BIOL343_TUT02-CC2_VictoriaGutierrez)
3. **Margo Kaminska** - 20449041
 + [BIOL343_CC](https://github.com/margo-kaminska/BIOL343)
4. **Emma Popple** — 20467453
 + [BIOL343_CC](https://github.com/emmapopple/BIOL343_CC)
5. **Kaylee Ellwood** - 20391007
 + [BIOL343_CC2](https://github.com/kaylee61/BIOL343_CC2.git)
6.  **Téa Hope** — 20459579
+ [BIOL343_CC](https://github.com/teahope20/BIOL343.git)
...
positpal::start("SabrinaBaker_CC2.Rmd") 
...
-------|-------|-------
Population Type | Either sexual (S) or asexual (A) | NA
Ovary Width | Width of a given plant | mm
Ovary Height | Height of a given plant | mm
Flower Number | Number of flowers on a given plant | Counts
...
```{r import-clean}
cc1_data <- read.csv("A1_Data_Excel copy.csv") 
new_cc1_data <- subset(cc1_data[1:46, 1:8]) 
new_cc1_data

width <- new_cc1_data$Ovary.Width..mm.
height <- new_cc1_data$Ovary.Height..mm.
ovary_area <- width*height 
ovary_area
```
...
Both ovary width and height were measured using $mm$, the final unts for area is $mm^2$ 
...
names(new_cc1_data)
 
summary(new_cc1_data$ovary_area)
 
table(new_cc1_data$Population.Type, useNA = "ifany")
 
sum(!is.na(new_cc1_data$ovary_area))
sum(!is.na(new_cc1_data$Population.Type)) 


```{r area-means, include=FALSE}

area_means <- aggregate (ovary_area ~ Population.Type, data=new_cc1_data, FUN=mean) 
area_means
...
library(knitr)
kable(area_means, caption = "Mean ovary areas (mm^2)") 
...
```{r area-histogram, echo=FALSE, message=FALSE, warnings=FALSE}

library(ggplot2)
Plot1 <- ggplot(aes(x=ovary_area), data=new_cc1_data)  + 
   geom_histogram (binwidth = 0.8) +
   facet_wrap(Population.Type~.) + 
   labs(title = "Distribution of Ovary Areas", x = "Ovary Area (mm)", y = "Frequency") +
   theme_classic()
...
![Solanum pimpinellifolium](images/S.pimp.png)

Source: [Kew: Royal Botanic Gardens](https://powo.science.kew.org/taxon/urn:lsid:ipni.org:names:820511-1/images) 
